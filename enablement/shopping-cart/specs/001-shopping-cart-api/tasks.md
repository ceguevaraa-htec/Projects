...
- [X] T005 Define the `Cart`, `LineItem`, and `CartStatus` (`"open" | "checked_out"`) types in
      `src/models/cart.ts`, per data-model.md
- [X] T006 [P] Implement a minimal structured logger in `src/logger.ts` exposing a function
...
- [X] T007 [P] Define typed error classes in `src/http.ts`: `CartNotFoundError`,
...
- [X] T008 Implement the carts repository in `src/repositories/carts.repository.ts`: an
...
- [X] T009 Create the Express app skeleton in `src/app.ts`: JSON body parsing middleware, a
      ...
      (depends on T007)
- [X] T010 Create the process entrypoint in `src/server.ts` that imports the app from
...
- [X] T011 [P] [US1] Unit tests for create/add/get in `tests/unit/carts.service.test.ts`:
...
- [X] T012 [P] [US1] Integration tests for `POST /carts`, `POST /carts/:cartId/items`,
...
- [X] T013 [US1] Implement `createCart()`, `addItem(cartId, productId, quantity, price)`, and
...
- [X] T014 [US1] Implement `POST /carts`, `POST /carts/:cartId/items`, `GET /carts/:cartId` in
...
- [X] T015 [US1] Mount the carts router from T014 onto the Express app in `src/app.ts` at the
      `/carts` base path
...
- [X] T016 [P] [US2] Unit tests for update/remove in `tests/unit/carts.service.test.ts`:
...
- [X] T017 [P] [US2] Integration tests for `PATCH /carts/:cartId/items/:productId` and
...
- [X] T018 [US2] Implement `updateItemQuantity(cartId, productId, quantity)` and
...
- [X] T019 [US2] Implement `PATCH /carts/:cartId/items/:productId` and
...
- [X] T020 [P] [US3] Unit tests for checkout/clear in `tests/unit/carts.service.test.ts`:
...
- [X] T021 [P] [US3] Integration tests for `POST /carts/:cartId/checkout` and
...
- [X] T022 [US3] Implement `checkout(cartId)` and `clearCart(cartId)` in
...
- [X] T023 [US3] Implement `POST /carts/:cartId/checkout` and `POST /carts/:cartId/clear` in
...
- [X] T024 [P] Run every scenario in `quickstart.md` against the running dev server (`npm run
      dev` + the curl commands) and confirm the observed status codes/bodies match
      contracts/cart-api.md
- [X] T025 [P] Add a `README.md` at repo root documenting how to install, run (`npm run dev`),
      test (`npm test`), and the seven available endpoints
- [X] T026 Review the logger output (T006) across `createCart`, `addItem`,
...
- [X] T027 [P] Add an integration test in `tests/integration/carts.router.test.ts` confirming a
